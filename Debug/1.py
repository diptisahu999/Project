...
# def tcp_connections():
#     # tcp_devices = json_opr.getJson(param.site_json)
#     # tcp_devices = Devices.objects.filter(device_type='DIP',app_type='AVR')
#     # for tcp in tcp_devices["devices"]:
#     for tcp in tcp_devices:
#         # print(tcp)
#         # if tcp['device_type'] == 'DIP':
#         #     if tcp['app_type'] == "AVR":
#         #         tcp_details = tcpevice_object["ip"],tcp.device_object["port"],tcp.device_type,tcp.app_type,tcp.id)
#         try:
#             tcp_record = Tcp_opr(tcp.device_name,tcp.device_object["ip"],tcp.device_object["port"],tcp.device_type,tcp.app_type,tcp.id)
#             config.site_tcp.append(tcp_record)
#         except Exception:
#             logging.info("Something went wrong connecting " + tcp.device_object["ip"] + ":" + tcp.device_object["port"])

#     # for thread in config.tcp_listener_threads:
#     #     thread.start()

# class Tcp_opr:
#     def __init__(self,name, ip_address, port, device_type, app_type, id):
#         self.ID = id
#         self.CM_NAME = name
...
def operation(self, value):
    try:
        logging.info(f"{self.APP_TYPE} operation {value}")
        self.t.sendto(str(value).encode('utf-8'), (self.HOST_ADDRESS, int(self.HOST_PORT)))
        # param.logUpdate("AVR Operation : " + value)
    except Exception as e:
        self.connected = False
        logging.info(f"{self.APP_TYPE} connection error while sending {value}")  
        logging.error(f"{self.APP_TYPE} Error {e}")         
    return

def tenways_reader(self):
    if self.connected:
        try:
            # self.t.sendto(str("MV?").encode('utf-8'), (self.HOST_ADDRESS, int(self.HOST_PORT)))
            data,sender  = self.t.recvfrom(1024)
            return data
        except:
            self.connected = False  
    ...
    #     time.sleep(2)
    #     logging.info(f"{self.APP_TYPE} disconnected {self.HOST_ADDRESS}:{self.HOST_PORT}")
    #     self.connect()
    #     return b'>'
...
# param=[{"device_id":61,
#        "channel_id":10,
#        "device_status":"true"},{"device_id":55,
#        "channel_id":13,
#        "device_status":"true"}]
# # for i in param:
#     relay_opr(i)
